...
kernel_size = 3      # Size of the convolutional window (looks at 3 tokens at a time)

# Training Configuration
num_epochs = 20
learning_rate = 1e-3
batch_size = 4          # How many sequences to process in parallel on the GPU
...
# THE CUSTOM NON-LINEARITY (Our Special Sauce) ---
...
def safe_decode(enc, indices):
    """
    Decodes a list of token indices, ignoring any that cause a KeyError.
    This is essential for handling raw model outputs that might contain
    special or reserved tokens.
    """
    byte_chunks = []
    for token in indices:
        try:
            ...
            # Ignore tokens that are not in the decoder's vocabulary
            pass
...
def train_llm():
    ...
    data = torch.tensor(enc.encode_ordinary(corpus), dtype=torch.long) # Use encode_ordinary to avoid special tokens in training data
    n = int(0.9 * len(data))
    ...
    print("--- Deep Causal CNN LLM Initialized ---")
